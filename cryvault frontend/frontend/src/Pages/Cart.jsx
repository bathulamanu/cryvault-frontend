...
const Cart = () => {
  ...
  const [exampleData, setExampleData] = useState(sessionStorage.getItem("exampleData"));
  ...
  useEffect(() => {
    const storedExampleData = sessionStorage.getItem("exampleData");
    const storedAmount = sessionStorage.getItem("amount");

    if (storedExampleData !== exampleData) {
      setExampleData(storedExampleData);
    }
  ...
  const deleteitem = () => {
    setExampleData(sessionStorage.removeItem("exampleData"));
    setAmount(sessionStorage.removeItem("amount"));
  ...
  return (
    <>
      <Box sx={{ marginTop: "5rem", flexDirection: isMobile ? "column" : "row", padding: isMobile ? "0rem 2rem" : " 9rem 10rem" }} className="cart">
          ...
          <Box className={!isMobile ? "cartProducts" : "cartProductsMobile"}>
            <Box sx={{ display: "flex", gap: "2rem" }}>
              ...
              <Box className="productDetails">
                <Typography sx={{ fontSize: "2.5rem", fontWeight: "500" }}> Basic</Typography>
                <Typography sx={{ fontSize: "2.5rem", fontWeight: "500" }}>Basic details</Typography>
              </Box>
            </Box>

            <Box className="productAmount">
              <Typography sx={{ fontSize: "2.5rem", fontWeight: "500", whiteSpace: "nowrap" }}>{parseInt(124124)} INR</Typography>
